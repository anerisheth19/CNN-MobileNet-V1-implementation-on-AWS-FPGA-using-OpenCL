...
int main(int argc, char** argv) {
	...
	// Create the compute kernel in the program we wish to run
	kernel = clCreateKernel(program, "depthwise2", &err);
	if (!kernel || err != CL_SUCCESS)
	{
		printf("Error: Failed to create compute kernel!\n");
		exit(1);
	...
	d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, FILTER_SIZE_L4*K*K*sizeof(int), filter, &err);	
	...
	err |= clEnqueueWriteBuffer(commands, d_filter, CL_TRUE, 0, FILTER_SIZE_L4*K*K*sizeof(int), filter, 0, NULL, NULL);
	...
	readSquezeNetKernel(filter, (FILTER_SIZE_L5*FILTER_SIZE_4*K_P*K_P));
	...
	d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, FILTER_SIZE_L5*FILTER_SIZE_L4*K_P*K_P*sizeof(int), filter, &err);	
	...
	err |= clEnqueueWriteBuffer(commands, d_filter, CL_TRUE, 0, FILTER_SIZE_L4*FILTER_SIZE_L5*K_P*K_P*sizeof(int), filter, 0, NULL, NULL);
	...
	d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, FILTER_SIZE_L6*K*K*sizeof(int), filter, &err);	
	...
	err = clEnqueueWriteBuffer(commands, d_image_l6, CL_TRUE, 0, (HEIGHT/2)*(WIDTH/2)*FILTER_SIZE_L4*sizeof(unsigned char), output_image_l5, 0, NULL, NULL);
	err |= clEnqueueWriteBuffer(commands, d_filter, CL_TRUE, 0, FILTER_SIZE_L6*K*K*sizeof(int), filter, 0, NULL, NULL);
	...
	clGetEventProfilingInfo(myevent,CL_PROFILING_COMMAND_END,sizeof(cl_ulong), &end, NULL);
	kernelExecTimeNs += end - start;
	err = clEnqueueReadBuffer(commands, d_output, CL_TRUE, 0, FILTER_SIZE_L4*(HEIGHT/4)*(WIDTH/4)*sizeof(unsigned char), output_image_l6, 0, NULL, NULL);
	...
	d_output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, (HEIGHT/4)*(WIDTH/4)*FILTER_SIZE_L6*sizeof(unsigned char), NULL, &err);
	d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, FILTER_SIZE_L6*FILTER_SIZE_L6*K_P*K_P*sizeof(int), filter, &err);	
	...
	err |= clEnqueueWriteBuffer(commands, d_filter, CL_TRUE, 0, FILTER_SIZE_L6*FILTER_SIZE_L6*K_P*K_P*sizeof(int), filter, 0, NULL, NULL);
	...
	d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, FILTER_SIZE_L6*FILTER_SIZE_L9*K_P*K_P*sizeof(int), filter, &err);	
	...
	err = clEnqueueWriteBuffer(commands, d_image_l9, CL_TRUE, 0, (HEIGHT/8)*(WIDTH/8)*FILTER_SIZE_L9*sizeof(unsigned char), output_image_l8, 0, NULL, NULL);
	err |= clEnqueueWriteBuffer(commands, d_filter, CL_TRUE, 0, FILTER_SIZE_L6*FILTER_SIZE_L9*K_P*K_P*sizeof(int), filter, 0, NULL, NULL);
	...
	d_output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, (HEIGHT/8)*(WIDTH/8)*FILTER_SIZE_L9*sizeof(unsigned char), NULL, &err);
	d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, FILTER_SIZE_L9*K*K*sizeof(int), filter, &err);	
	...
	err |= clEnqueueWriteBuffer(commands, d_filter, CL_TRUE, 0, FILTER_SIZE_L9*K*K*sizeof(int), filter, 0, NULL, NULL);
	...
	d_output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, (HEIGHT/8)*(WIDTH/8)*FILTER_SIZE_L9*sizeof(unsigned char), NULL, &err);
	d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, FILTER_SIZE_L9*FILTER_SIZE_L9*K_P*K_P*sizeof(int), filter, &err);	
	...
	err |= clEnqueueWriteBuffer(commands, d_filter, CL_TRUE, 0, FILTER_SIZE_L9*FILTER_SIZE_L9*K_P*K_P*sizeof(int), filter, 0, NULL, NULL);
	...
	d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, FILTER_SIZE_L9*FILTER_SIZE_L13*K_P*K_P*sizeof(int), filter, &err);	
	...
	err |= clEnqueueWriteBuffer(commands, d_filter, CL_TRUE, 0, FILTER_SIZE_L9*FILTER_SIZE_L13*K_P*K_P*sizeof(int), filter, 0, NULL, NULL);
	...
	free(output_image_l4);
	free(output_image_l5);
 	free(output_image_l5);
	free(output_image_l6);
	free(output_image_l7);
	free(output_image_l8);
	free(output_image_l9);
	free(output_image_l10);
	free(output_image_l11);
	free(output_image_l12);
	free(output_image_l13);
	...
	clReleaseMemObject(d_image_l12);
